...
app.post('/uploaddata', (req, res) => {
	...
	db.collection('postcounter').findOne({ name: 'total' }, (error, result) => {
		...
		db.collection('contact').insertOne({ _id : (parseInt(totalResult) + 1), date: nowLocale, company: req.body.company, bn: req.body.bn, phone: req.body.phone, category: req.body.category,
			another: req.body.another, payment: req.body.payment, url: req.body.url, max: (req.body.max + '백만원'), saveFileName: copy, destructionDate : destructionDate, read : 'off', adminMemo: ''}, (error, result) => {
