...
const getLocale = () => {
	var locale = new Date().toLocaleString();
	return locale;
}
const getUTC = () => {
...
var destructionDate = getUTC() + 31536000000;
...
var storage = multer.diskStorage({
	...
	filename: function (req, file, cb) {
		cb(null, getUTC() + (file.originalname = Buffer.from(file.originalname, 'latin1').toString('utf8')))
	}
});
...
var upload = multer({
	...
	fileFilter: (req, file, callback) => {
		var ext = path.extname(getUTC() + file.originalname);
...
app.post('/uploaddata', (req, res) => {
	...
	for (var i = 0; i < copy.length; i++) {
		copy[i] = getUTC() + req.body.fileName[i];
	}

	db.collection('postcounter').findOne({ name: 'total' }, (error, result) => {
		var totalResult = result.totalPost;
		db.collection('contact').insertOne({
			_id: (parseInt(totalResult) + 1), date: getLocale(), company: req.body.company, bn: req.body.bn, phone: req.body.phone, category: req.body.category, another: req.body.another,
			payment: req.body.payment, url: req.body.url, max: parseInt(req.body.max), saveFileName: copy, destructionDate: destructionDate, read: 'off', adminMemo: '', utc : getUTC()
		}, (error, result) => {
...
const updateVisitorHistory = schedule.scheduleJob('0 59 23 * * *', () => {
	db.collection('todaycounter').findOne({ name: 'today' }, (error, result) => {
		db.collection('visitorhistory').insertOne({ date : getLocale()
			, pc : result.pc, mobile : result.mobile, total : result.total, utc : getUTC()}, (error, result) => {		
		});
	});
	console.log('업뎃 실행');
});
